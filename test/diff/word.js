...
describe("WordDiff", function () {
  describe("#diffWords", function () {
    ...
    it("should token unicode characters safely", function () {
      expect(wordDiff.removeEmpty(wordDiff.tokenize("jurídica"))).to.eql([
        "jurídica",
      ]);
      expect(wordDiff.removeEmpty(wordDiff.tokenize("wir üben"))).to.eql([
        "wir",
        " ",
        "üben",
      ]);
    ...
    it("should include count with identity cases", function () {
      ...
      expect(diffWords("foo bar", "foo bar")).to.eql([
        { value: "foo bar", count: 3 },
      ]);
    });
    it("should include count with empty cases", function () {
      expect(diffWords("foo", "")).to.eql([
        { value: "foo", count: 1, added: undefined, removed: true },
      ]);
      expect(diffWords("foo bar", "")).to.eql([
        { value: "foo bar", count: 3, added: undefined, removed: true },
      ]);

      expect(diffWords("", "foo")).to.eql([
        { value: "foo", count: 1, added: true, removed: undefined },
      ]);
      expect(diffWords("", "foo bar")).to.eql([
        { value: "foo bar", count: 3, added: true, removed: undefined },
      ]);
    });

    it("should ignore whitespace", function () {
      expect(diffWords("hase igel fuchs", "hase igel fuchs")).to.eql([
  ...
  describe("#diffWordsWithSpace", function () {
    ...
    it("should threat newline as separate token (issues #180, #211)", function () {
      ...
      expect(convertChangesToXML(diffResult1)).to.equal(
        "foo\n<ins>\n\n</ins>bar"
      );
      // #211
    ...
    describe("case insensitivity", function () {
      ...
      it("is considered when there's no difference", function () {
        const diffResult = diffWordsWithSpace("new value", "New Value", {
          ignoreCase: true,
        });
        expect(convertChangesToXML(diffResult)).to.equal("New Value");
      });
